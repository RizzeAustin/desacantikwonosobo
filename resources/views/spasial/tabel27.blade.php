...
<div class="row mt-4">
    <div class="col-xl">
        <div class="card mb-4 border-dark">
            ...
            <div class="card-body">
                <form action="{{ url('/spasial/tabel27/add') }}" method="POST" id="form" class="form-spasial" autocomplete="off">
                    @csrf
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-5">
                            <div class="form-floating">
                                <select type="text" class="form-select" id="k2" name="k2" placeholder="k2">
                                    <option value=""></option>
                                    <option value="1">1. TPA </option>
                                    <option value="2">2. Tempat Pembuangan Sampah </option>
                                    <option value="3">3. Gerobak Sampah </option>
                                    <option value="4">4. Icinerator </option>
                                    <option value="5">5. Tong Sampah </option>
                                    <option value="6">6. Bank Sampah </option>
                                    <option value="7">7. Satgas Kebersihan </option>
                                </select>
                                <label for="k2" class="form-label">2. Jenis Fasilitas Pengolahan Sampah</label>
                        ...
                        <div class="col-md-6">
                            <div class="form-floating">
                                ...
                                <label for="k3" class="form-label">3. Nama Pengurus</label>
        ...
        <table id="dataTable" class="table table-bordered table-stripped display nowrap">
            <thead>
                <tr class="v-mid t-center">
                    <th>No</th>
                    <th>Jenis Fasilitas Pengolahan Sampah</th>
                    <th>Nama Pengurus</th>
                    <th>Dusun</th>
                    <th>RW</th>
                    <th>RT</th>
                ...
                @foreach ($data as $d)
                    <tr class="v-mid t-center">
                        ...
                        <td>
                            @switch($d->jenis)
                                @case('1') 1. TPA @break
                                @case('2') 2. Tempat Pembuangan Sampah @break
                                @case('3') 3. Gerobak Sampah @break
                                @case('4') 4. Icinerator @break
                                @case('5') 5. Tong Sampah @break
                                @case('6') 6. Bank Sampah @break
                                @case('7') 7. Satgas Kebersihan @break
                                @default {{ $d->jenis }}
                            @endswitch
                        </td>
                        <td>{{ $d->pengurus }}</td>                     
...
<script>
                    ...
                    if($.isEmptyObject(data.error)){
                        ...
                        $("#dataTable tbody tr").each(function () {
                            ...
                            if (no==$('#k1').val()){
                                ...
                                t.find("td:eq(3)").html($('#dusun option:selected').text())
                                t.find("td:eq(4)").html($('#rw option:selected').text())
                                t.find("td:eq(5)").html($('#rt option:selected').text())
                                t.find("td:eq(6)").html($('#flag option:selected').text())
                                return false
                            }
                        });
                        if (tidakAda){
    ...
    function edit(id){
        let url = `{{ url('/spasial/tabel27/data') }}`
        $.ajax({
            url: url,
            type: "POST",
            data:{ 
