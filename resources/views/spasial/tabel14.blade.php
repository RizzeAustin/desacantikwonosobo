...
<div class="row mt-4">
    <div class="col-xl">
        <div class="card mb-4 border-dark">
            ...
            <div class="card-body">
                <form action="{{ url('/spasial/tabel14/add') }}" method="POST" id="form" class="form-spasial" autocomplete="off">
                    @csrf
                    ...
                    <div class="row">
                        <div class="col-md-3">
                            <div class="form-floating">
                                <select type="text" class="form-select" id="k2" name="k2" placeholder="k2">
                                    <option value=""></option>
                                    <option value="1">1. Banjir </option>
                                    <option value="2">2. Tanah Longsor </option>
                                    <option value="3">3. Angin Puting Beliung </option>
                                    <option value="4">4. Kebakaran Hutan, </option>
                                    <option value="5">5. Gempa Bumi </option>
                                    <option value="6">6. Gunung Meletus </option>
                                    <option value="7">7. Tanah Longsor </option>
                                    <option value="8">8. Banjir </option>
                                    <option value="9">9. Banjir Bandang </option>
                                    <option value="10">10. Kekeringan </option>
                                    <option value="11">11. Kebakaran Hutan Dan Lahan, </option>
                                    <option value="12">12. Angin Puyuh/Putting </option>
                                    <option value="13">13. Beliung/Topan </option>
                                </select>
                                <label for="k2" class="form-label">2. Jenis Kejadian Bencana Alam</label>
                                <small><span class="text-danger error-text k2_err"></span></small>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="form-floating">
                                <input type="date" class="form-control" id="k3" name="k3" placeholder="k3" value="">
                                <label for="k3" class="form-label">3. Tanggal Kejadian</label>
                        ...
                        <div class="col-md-3">
                            <div class="form-floating">
                                ...
                                <label for="k4" class="form-label">4. Banyaknya Korban Jiwa</label>
                                <small><span class="text-danger error-text k4_err"></span></small>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="form-floating">
                                <input type="number" class="form-control" id="k5" name="k5" placeholder="k5" value="" min="0">
                                <label for="k5" class="form-label">5. Jumlah Kerugian</label>
        ...
        <table id="dataTable" class="table table-bordered table-stripped display nowrap">
            <thead>
                <tr class="v-mid t-center">
                    <th>No</th>
                    <th>Jenis Kejadian Bencana Alam</th>
                    <th>Tanggal Kejadian <br>( YYYY-MM-DD )</th>
                    <th>Banyaknya Korban Jiwa</th>
            ...
            <tbody>
                @foreach ($data as $d)
                    <tr class="v-mid t-center">
                        ...
                        <td>
                            @switch($d->jenis)
                                @case('1') 1. Banjir @break
                                @case('2') 2. Tanah Longsor @break
                                @case('3') 3. Angin Puting Beliung @break
                                @case('4') 4. Kebakaran Hutan, @break
                                @case('5') 5. Gempa Bumi @break
                                @case('6') 6. Gunung Meletus @break
                                @case('7') 7. Tanah Longsor @break
                                @case('8') 8. Banjir @break
                                @case('9') 9. Banjir Bandang @break
                                @case('10') 10. Kekeringan @break
                                @case('11') 11. Kebakaran Hutan Dan Lahan, @break
                                @case('12') 12. Angin Puyuh/Putting @break
                                @case('13') 13. Beliung/Topan @break
                                @default {{ $d->jenis }}
                            @endswitch    
                        </td>
                        <td>{{ $d->tanggal }}</td>
                        <td>{{ $d->jumlahkorban }}</td>
                        <td>{{ $d->jumlahkerugian }}</td>
                        <td>
...
<script>
    $(document).ready(function(){
        ...
        var dataTable = $('#dataTable').DataTable({
            ...
            initComplete: function () {
                this.api().columns([1,5,6,7,8]).every( function () {
                    var column = this;
                    ...
                    if($.isEmptyObject(data.error)){
                        ...
                        $("#dataTable tbody tr").each(function () {
                            ...
                            if (no==$('#k1').val()){
                                ...
                                t.find("td:eq(5)").html($('#dusun option:selected').text())
                                t.find("td:eq(6)").html($('#rw option:selected').text())
                                t.find("td:eq(7)").html($('#rt option:selected').text())
    ...
    function edit(id){
        let url = `{{ url('/spasial/tabel14/data') }}`
        $.ajax({
            url: url,
            type: "POST",
            data:{ 
            ...
            success: function(data){
                ...
                $('#k2').val(d.jenis)
                $('#k3').val(d.tanggal)
                $('#k4').val(d.jumlahkorban)
                $('#k5').val(d.jumlahkerugian)
